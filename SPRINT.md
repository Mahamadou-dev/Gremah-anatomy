...
>
> **Dernière vérification :** 21 août 2026. Sprint en cours : **14** (`sprint-14/e2e`, rituel de clôture non déroulé).
...
| 14     | 🟨 en cours | `sprint-14/e2e`          | Playwright installé et câblé en CI (job `e2e`, Chromium desktop + mobile) : smoke sur les pages publiques, bascule de thème, garde de `/atlas` et `/profil`, parcours inscription→atlas→déconnexion, cycle complet du profil (modifier/exporter/supprimer), et un test qui rend vraiment le canevas 3D et échoue sur toute erreur console. Ajouté cette passe : capture WebGL de référence (`e2e/rendu-3d.spec.ts`, `toHaveScreenshot` avec `maxDiffPixelRatio` explicite), test de repli WebGL2 forcé via `?renderer=webgl` (vérifie l'absence d'écran noir et d'erreur console — charte §5.1), suite `@axe-core/playwright` sur les routes publiques (`e2e/accessibilite.spec.ts` — a trouvé et corrigé un vrai défaut : le tableau `/credits/` n'était pas accessible au clavier, `app/credits/page.tsx` + `globals.css`), et budget Lighthouse en CI (`lighthouserc.cjs`, job `lighthouse`, cible `/` et `/atlas/` — `/atlas/` redirige vers `/connexion/` sans secrets de compte posés, mesuré honnêtement tel quel). Dépendances ajoutées et justifiées : `@axe-core/playwright`, `@lhci/cli` (outillage officiel, pas de service tiers). Vérifié en local contre Atlas ; en CI, `comptes.spec.ts` se saute tant que `MONGODB_URI`/`AUTH_SECRET` ne sont pas posés en secrets GitHub (§ À faire à la main). **Reste : banc mémoire (`e2e/memoire.spec.ts`) — écrit, en cours de vérification finale contre Atlas.** |
