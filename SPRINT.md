...
| 14     | 🟨 en cours | `sprint-14/e2e`          | Playwright installé et câblé en CI (job `e2e`, Chromium desktop + mobile) : smoke sur les pages publiques, bascule de thème, garde de `/atlas` et `/profil`, parcours inscription→atlas→déconnexion, cycle complet du profil (modifier/exporter/supprimer), et un test qui rend vraiment le canevas 3D et échoue sur toute erreur console. Vérifié en local contre Atlas ; en CI, `comptes.spec.ts` se saute tant que `MONGODB_URI`/`AUTH_SECRET` ne sont pas posés en secrets GitHub (§ À faire à la main). **Reste : captures WebGL de référence, chemin de repli WebGL2 forcé, budget Lighthouse, axe-core, banc mémoire.** |
